...
class HalideTempAddToSubReplacement : public IRMutation {
public:
  HalideTempAddToSubReplacement(llvm::Instruction::BinaryOps from, llvm::Instruction::BinaryOps to);
...
#define BINARY_REPLACEMENT(FROM, TO)                                                               \
  class FROM##HalideTo##TO : public HalideTempAddToSubReplacement {                                \
  public:                                                                                          \
    FROM##HalideTo##TO()                                                                           \
        : HalideTempAddToSubReplacement(llvm::Instruction::FROM, llvm::Instruction::TO) {}         \
  };
