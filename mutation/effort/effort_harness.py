...
TEST_KINDS = ["test1_demo", "test2_shipped", "test2_added", "test3_perf"]
...
CSV_FIELDS = [
    ...
    "test1_demo", "test2_shipped", "test2_added", "test2_method",
    "test3_perf",
    "killed_by", "exit_code", "wall_seconds", "perf_median",
    "artifact_digest", "after_exit", "after_digest", "after_seconds",
    "tool_id", "note",
]
...
def main(argv=None):
    ...
    ap.add_argument("--only-uids", default="",
                    help="file of mutant_uids, one per line; restricts the "
                         "sweep to those points")
    ap.add_argument("--after-driver", default="",
                    help="ADDED driver source, relative to --halide-root. "
                         "Swapped in with dataclasses.replace; no shipped "
                         "file is ever edited.")
    ap.add_argument("--after-args", default="[]",
                    help="JSON list of argv for the added driver")
    ap.add_argument("--after-artifact", default="out.bin",
                    help="artifact the added driver writes; empty means it "
                         "is scored on its exit status alone")
    ap.add_argument("--skip-before", action="store_true",
                    help="do not re-run the shipped driver (the BEFORE "
                         "verdict is then carried over from the prior "
                         "sweep rather than re-measured)")
    ...
    if a.only_uids:
        want = {ln.strip() for ln in Path(a.only_uids).read_text().splitlines()
                if ln.strip() and not ln.startswith("#")}
        keep = [p for p in points if p.uid in want]
        missing = want - {p.uid for p in keep}
        if missing:
            raise SystemExit(f"{a.app}: {len(missing)} requested uid(s) are "
                             f"not among this run's points, e.g. "
                             f"{sorted(missing)[:5]} -- the point set has "
                             f"moved and the before/after pairing would be "
                             f"against a different mutant")
        # Sorted by SOURCE POSITION, never completion order.
        points = sorted(keep, key=lambda p: (p.file, p.line, p.column,
                                             p.ordinal))
        log(f"[{a.app}] restricted to {len(points)} named mutant_uids")
    if a.limit:
    ...
    # ---------------------------------------------- test2 (ADDED) setup
    # The added driver is swapped in exactly the way experiment 2 does it:
    # dataclasses.replace on the app config. The shipped file on disk is
    # untouched, so the BEFORE verdict stays the one the sweep measured.
    after_app = None
    after_bin = None
    after_sig = None
    after_timeout = 600.0
    if a.after_driver:
        after_app = dataclasses.replace(
            appcfg,
            driver_source=a.after_driver,
            driver_args=json.loads(a.after_args),
            output_artifact=(a.after_artifact or None),
            extra_driver_outputs=[],
        )
        adrv_obj = scratch / "obj" / "after_driver.o"
        if not pipe.compile_driver_object(after_app, base_art, adrv_obj):
            raise SystemExit(f"{a.app}: ADDED driver {a.after_driver} failed "
                             f"to compile")
        after_bin = scratch / "obj" / "baseline.after"
        if not pipe.build_driver(after_app, base_art, after_bin, adrv_obj):
            raise SystemExit(f"{a.app}: ADDED driver failed to link")
        asigs, atimes = [], []
        acfg = dataclasses.replace(after_app, run_timeout=600)
        for i in range(a.baseline_runs):
            rd = scratch / "run" / f"bafter-{i}"
            code, sout, secs, timed = pipe.run_driver(acfg, after_bin, rd)
            if timed or code != 0:
                log(sout[-2000:])
                raise SystemExit(
                    f"{a.app}: the ADDED test does not pass on the UNMUTATED "
                    f"pipeline (exit {code}, timeout={timed}). A test that "
                    f"already fails clean cannot be used to kill anything.")
            if after_app.output_artifact and \
                    not (rd / after_app.output_artifact).exists():
                raise SystemExit(
                    f"{a.app}: the ADDED test exited 0 but wrote no "
                    f"{after_app.output_artifact}; an absent artifact would "
                    f"make every comparison verdict meaningless")
    ...
    def _run_after(row, art, abin):
        """Run the ADDED test against this mutant's artifacts.

        Scored on two mechanisms, both guarded against an absent artifact:

          assertion       the test's own checks fired and it exited nonzero
                          CLEANLY. A crash (negative code, i.e. a signal) is
                          not an assertion failure and is not scored here --
                          it wrote no artifact either, so it is a test1-style
                          observation, recorded in the note.
          output_compare  the artifact exists and differs from the golden the
                          baseline established over `--baseline-runs` runs.

        A timeout or a crash leaves test2_added at NOT_RUN, never 0.
        """
        if after_app is None:
            return row
        if not pipe.build_driver(after_app, art, abin, None):
            row["note"] = (row["note"] + "; ADDED driver link failed").strip("; ")
            return row
        ard = scratch / "run" / f"{row['mutant_uid']}-a"
        acfg = dataclasses.replace(after_app, run_timeout=int(after_timeout))
        acode, aout, asecs, atimed = pipe.run_driver(acfg, abin, ard)
        row["after_seconds"] = f"{asecs:.3f}"
        row["after_exit"] = "" if acode is None else acode
        try:
            if atimed:
                row["test2_added"] = "NOT_RUN"
                row["note"] = (row["note"] + "; ADDED test timed out: no exit "
                               "status and no artifact, so neither mechanism "
                               "returns a verdict").strip("; ")
                return row
            if acode is not None and acode < 0:
                row["test2_added"] = "NOT_RUN"
                row["note"] = (row["note"] + f"; ADDED test died on signal "
                               f"{-acode}, wrote no artifact -- not scored as "
                               f"an assertion failure").strip("; ")
                return row
            mech = []
            if acode != 0:
                mech.append("assertion")
            art_path = ard / after_app.output_artifact \
                if after_app.output_artifact else None
            if art_path is not None:
                if not art_path.exists():
                    row["note"] = (row["note"] + "; ADDED test wrote no "
                                   "artifact, comparison not run").strip("; ")
                else:
                    sig = pipe.oracle_signature(after_app, ard, aout)
                    row["after_digest"] = (sig or "")[:16]
                    if sig != after_sig:
                        mech.append("output_compare")
            row["test2_method"] = "+".join(mech)
            row["test2_added"] = "KILLED" if mech else "SURVIVED"
            return row
        finally:
            shutil.rmtree(ard, ignore_errors=True)

    def _after_only(p, row, art, abin):
        row["test2_shipped"] = "NOT_RUN"
        _run_after(row, art, abin)
        return row
    ...
    def evaluate(p: Point) -> dict:
        ...
        row.update(app=a.app, route="ast_mutator", target_kind="generator",
                   ...
                   test1_demo="NOT_RUN", test2_shipped="NOT_RUN",
                   test2_added="NOT_RUN",
        ...
        try:
            ...
            timed = False
            if a.skip_before:
                row["test1_demo"] = "NOT_RUN"
                row["note"] = ("BEFORE not re-measured in this run; the "
                               "shipped-driver verdict is the prior "
                               "sweep's")
                return _after_only(p, row, art, abin)
            ...
            if golden_sig is not None and not timed:
                if not pipe.build_driver(golden_app, art, gbin, None):
                    ...
                    if gtimed:
                        ...
                        row["test2_shipped"] = ("KILLED" if sig != golden_sig
                                                else "SURVIVED")
            ...
            for pth in (obj, genbin, binary, gbin, abin):
        ...
        for f in as_completed(futs):
            ...
            try:
                ...
                row.update(app=a.app, route="ast_mutator",
                           ...
                           test1_demo="NOT_RUN", test2_shipped="NOT_RUN",
                           test2_added="NOT_RUN", test3_perf="",
            ...
            with lock:
                ...
                if row["equivalent"] == "tce_lib":
                    C["tce_equiv"] += 1
                if row["test1_demo"] in ("KILLED", "SURVIVED") or \
                        row["test2_added"] in ("KILLED", "SURVIVED"):
                    C["run"] += 1
                if row["killed_by"]:
                    C["killed"] += 1
            done += 1
