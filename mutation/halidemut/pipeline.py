...
# Mutant env-var keys look like
#   <mutator-id>:<file>:<line>:<col>:<endline>:<endcol>
# Mull <= 0.21 recorded two identifiers per mutation point -- a short 4-field
# env-var key and this longer 6-field reporting identifier. Mull 0.34 collapsed
# them into the single 6-field form (MutationPoint::updateIdentifier), which is
# both what lands in .mull_mutants and what selects a mutant at run time.
MUTANT_KEY_RE = re.compile(
    r"^(Halide_[a-z_0-9]+):(/[^:]+):(\d+):(\d+):(\d+):(\d+)$")
...
class Pipeline:
    def __init__(
        ...
        # Mull names its frontends after the LLVM they are built against.
        self.llvm_version = os.environ.get("MULL_LLVM_VERSION", "20")
        ...
        self.include_dirs = [
            ...
            # An in-tree Halide build generates only Halide.h into build/include;
            # HalideRuntime.h and HalideBuffer.h ship as sources and are only
            # copied into an installed/distrib tree. Several apps (camera_pipe
            # via halide_trace_config.h, and every driver using Halide::Runtime
            # ::Buffer) include them directly.
            str(self.halide_root / "src" / "runtime"),
        ]
    ...
    def stage1(self, app: AppConfig, arm: str, mutators: List[str],
        ...
        if route == "ir":
            plugin = [f"-fpass-plugin={self.mull_output / f'mull-ir-frontend-{self.llvm_version}'}"]
        elif route == "ast":
            plugin = [f"-fplugin={self.mull_output / f'libmull-cxx-ast-frontend-{self.llvm_version}.so'}"]
        else:
